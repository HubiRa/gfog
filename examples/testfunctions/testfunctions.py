...
class TestFunctionBase(ABC):
    ...
    def plot3D(
        ...
        cmap: cm = sns.cubehelix_palette(start=.5, rot=-.75, as_cmap=True),
        path_style: str = "r.-" 
    ) -> None:
        X, Y = torch.meshgrid(x_range, y_range)
        Z = torch.tensor(self(torch.stack([X, Y], dim=-1).view(-1, 2))).view(len(x_range), len(y_range))
        ...
        surf = ax.plot_surface(X, Y, Z, cmap=cmap, linewidth=0.5, antialiased=False, alpha=0.8)
...
class BealeFunction(TestFunctionBase):
    def __init__(self) -> None:
        ...
        self.set_domain(
            lower=torch.tensor([-4.5, -4.5]), 
            upper=torch.tensor([4.5, 4.5])
        )
...
class GoldsteinPriceFunction(TestFunctionBase):
    def __init__(self) -> None:
        super().__init__(input_layout="BXY")
        self.set_domain(
            lower=torch.tensor([-2, -2]), 
            upper=torch.tensor([2, 2])
        )
...
def ackley_function(vec: torch.Tensor) -> torch.Tensor:
    ...
    return -20 * torch.exp(-0.2 * torch.sqrt(0.5 * (x ** 2 + y ** 2))) - torch.exp(
        0.5 * (torch.cos(2 * np.pi * x) + torch.cos(2 * np.pi * y))
    ) + np.e + 20
...
def holder_table_function(vec: torch.Tensor) -> torch.Tensor:
    ...
    y = vec[..., 1]
    return -torch.abs(
        torch.sin(x) * torch.cos(y) * torch.exp(
            torch.abs(1 - torch.sqrt(x ** 2 + y ** 2) / np.pi)
        )
    )
...
class HolderTableFunction(TestFunctionBase):
    def __init__(self) -> None:
        super().__init__(input_layout="BXY")
        self.set_domain(
            lower=torch.tensor([-10, -10]), 
            upper=torch.tensor([10, 10])
        )
...
class EasomFunction(TestFunctionBase):
    def __init__(self) -> None:
        super().__init__(input_layout="BXY")
        self.set_domain(
            lower=torch.tensor([-100, -100]), 
            upper=torch.tensor([100, 100])
        )
...
def cross_in_tray_function(vec: torch.Tensor) -> torch.Tensor:
    x = vec[..., 0]
    y = vec[..., 1]
    return -0.0001 * torch.abs(
        torch.sin(x) * torch.sin(y) * torch.exp(
            torch.abs(100 - torch.sqrt(x ** 2 + y ** 2) / np.pi)
        ) + 1
    ) ** 0.1   
