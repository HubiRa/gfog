...
from gfog.buffer import HirarchicalySortedBuffer
...
TEST_FUNCTION = MishrasBirdFunctionConstraint()
...
G = MLP(input_dim=LATENT_DIM, output_dim=F_DIM, hidden_dims=[32, 32]).to(GAN_DEVICE)
D = MLP(input_dim=F_DIM, output_dim=1, hidden_dims=[32, 32], spectral_norm=False).to(
    GAN_DEVICE
)


buffer = components.BufferComp(
    B=HirarchicalySortedBuffer(buffer_size=2 * BATCH_SIZE, value_levels=2)
)
...
gan = components.GAN(
    ...
    latent_sampler=LatentSamplerLambda(
        lambda b, d: torch.rand(b, d) * 2 - 1, b=BATCH_SIZE, d=LATENT_DIM
    ),
    device=GAN_DEVICE,
    dtype=torch.float,
)
