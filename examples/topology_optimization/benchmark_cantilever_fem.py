...
from cantilever_fem import build_parser, run_experiment


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark GFog FEM cantilever")
    parser.add_argument(
        "--curiosity_values",
        nargs="+",
        type=float,
    ...
    for curiosity in known_args.curiosity_values:
        for seed in known_args.seeds:
            ...
            console.rule(f"curiosity={curiosity} seed={seed}")
            results.append(run_experiment(args))

    summary = Table(title="FEM Cantilever Benchmark")
    ...
    summary.add_column("median best rel. compliance", justify="right")
    summary.add_column("median mean top-k rel. compliance", justify="right")
    ...
    for curiosity in known_args.curiosity_values:
        ...
        summary.add_row(
            ...
            f"{np.median([row['best_relative_compliance'] for row in rows]):.4f}",
            f"{np.median([row['mean_relative_compliance_topk'] for row in rows]):.4f}",
