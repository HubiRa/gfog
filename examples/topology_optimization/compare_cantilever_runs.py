...
def summarize(path: Path) -> dict[str, float | np.ndarray | str]:
    ...
    return {
        ...
        "backend": backend,
        "best_compliance": float(values[0, 2]),
        "mean_compliance_topk": float(values[:, 2].mean()),
...
def make_side_by_side(
    left: dict[str, float | np.ndarray | str],
    right: dict[str, float | np.ndarray | str],
    output_path: Path,
) -> None:
    ...
    headers = [
        (
            f"{left['backend']} curiosity={left['curiosity']:.0f}\n"
            f"best comp={left['best_compliance']:.4f}\n"
            f"L2={left['mean_l2']:.4f}, ham={left['mean_hamming']:.4f}"
        ),
        (
            f"{right['backend']} curiosity={right['curiosity']:.0f}\n"
            f"best comp={right['best_compliance']:.4f}\n"
    ...
    for row in range(rows):
        for col, run in enumerate([left, right]):
            ...
            ax.set_yticks([])
            ax.set_title(
                f"#{row + 1}: comp={values[row, 2]:.2f}\nvol={values[row, 0]:.3f} rough={values[row, 1]:.3f}",
                fontsize=9,
            )

    fig.text(0.25, 0.995, headers[0], ha="center", va="top", fontsize=11)
    fig.text(0.75, 0.995, headers[1], ha="center", va="top", fontsize=11)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare two structural cantilever runs"
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/structural_cantilever/comparison.png"),
    )
    ...
    logger.info(
        f"Left run: backend={left['backend']} curiosity={left['curiosity']:.0f} "
        f"best_comp={left['best_compliance']:.4f} mean_l2={left['mean_l2']:.4f}"
    )
    logger.info(
        f"Right run: backend={right['backend']} curiosity={right['curiosity']:.0f} "
        f"best_comp={right['best_compliance']:.4f} mean_l2={right['mean_l2']:.4f}"
    )
