...
from toy_topopt import build_parser, run_experiment


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark GFog toy topopt")
    parser.add_argument(
        "--curiosity_values",
        nargs="+",
        type=float,
    ...
    summary = Table(title="Toy TopOpt Benchmark")
    summary.add_column("curiosity")
    summary.add_column("seeds")
    summary.add_column("median best objective", justify="right")
    summary.add_column("median mean top-k objective", justify="right")
    ...
    for curiosity in known_args.curiosity_values:
        ...
        summary.add_row(
            f"{curiosity:.2f}",
            str(len(rows)),
            f"{np.median([row['best_objective'] for row in rows]):.4f}",
            f"{np.median([row['mean_objective_topk'] for row in rows]):.4f}",
