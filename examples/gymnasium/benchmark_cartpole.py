...
from cartpole import build_parser, run_experiment


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark GFog on CartPole")
    parser.add_argument(
        "--optimizers",
        nargs="+",
        default=["default", "wgangp"],
        choices=["default", "hinge", "wgan", "wgangp"],
    )
    parser.add_argument("--curiosity_values", nargs="+", type=float, default=[0.0, 5.0])
    ...
    summary = Table(title="CartPole GFog Benchmark")
