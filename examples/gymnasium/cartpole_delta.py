    ...
    def __call__(self, theta):
        ...
        for t in theta:
            ...
            for _ in range(self.runs_per_env):
                ...
                runs.append(total)
            returns.append(-np.median(runs))  # minimize
        return returns
...
# Clear the initial buffer (filled with raw G outputs) and seed departure points
buffer.B.clear()
n_departure_points = 1
optimizer.init_departure_points(dep_points=None, n_points=n_departure_points)

logger.info(
    f"Performance of best 5 samples in buffer after seeding (optimum: -{N_EPISODE_STEPS})"
)
