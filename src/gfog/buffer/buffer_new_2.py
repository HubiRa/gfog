...
class Buffer:
    ...
    #     self.tensors = [self.tensors[i] for i in idx_sorted]
    ...
    def insert(self, tensor: Tensor, value: float | Iterable[float]) -> None:
        ...
        self.buffer_core.insert(value_vec, tensor)
    ...
    def insert_many(
        ...
        self.buffer_core.insert_many(processed_values, tensors)
    ...
    def get(self, idx: int | slice) -> Tensor | List[Tensor]:
        if isinstance(idx, int):
            ...
            if idx < 0 or idx >= self.buffer_core.len():
                raise IndexError(f"Index {idx} out of bounds for buffer of length {self.buffer_core.len()}")
            tensor = self.buffer_core.get_tensor(idx)
            if tensor is None:
                raise IndexError(f"No tensor at index {idx}")
            return tensor
        elif isinstance(idx, slice):
            start, stop, step = idx.indices(self.buffer_core.len())
            tensors = []
            for i in range(start, stop, step):
                tensor = self.buffer_core.get_tensor(i)
    ...
    def get_random_batch(self, batch_size: int) -> List[Tensor]:
        return torch.stack(sample(self.tensors, batch_size))
    ...
    def get_value(self, index: int, level: int = 0) -> float:
        ...
        if index < 0 or index >= self.buffer_core.len():
            raise IndexError(f"Index {index} out of bounds for buffer of length {self.buffer_core.len()}")
        
        if level < 0:
            level = len(self.value_levels) + level
        if level < 0 or level >= len(self.value_levels):
            raise IndexError(f"Level index {level} out of bounds for {len(self.value_levels)} levels")
